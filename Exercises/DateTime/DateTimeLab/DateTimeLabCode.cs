...
namespace DateTimeLab
{
    public class DateTimeLabCode
    {
        ...
        {
            return DateTime.Parse(date).ToString("M.dd.yyyy hh:mm tt");
        }
        ...
        public DateTime[] GetNextWednesdays(int count, string startDate)
        {
            ...
            // GET FIRST WEDNESDAY
            while (parsedStartDate.DayOfWeek != DayOfWeek.Wednesday)
            {
                // ADD 1 DAY TO DATE AND OVERWRITE THE DATETIME OBJECT WITH THE NEW DATE
                parsedStartDate = parsedStartDate.AddDays(1);
            }

            // ADD FIRST WEDNESDAY TO ARRAY AT INDEX 0
            nextWednesdays[0] = parsedStartDate;

            // ADD THE REMAINING WEDNESDAYS TO ARRAY STARTING AT INDEX 1
            for (int i = 1; i < count; i++)
            {
                // ADD 7 DAYS TO DATE (TO GET THE NEXT WEDNESDAY) AND OVERWRITE THE DATETIME OBJECT WITH THE NEW DATE
                parsedStartDate = parsedStartDate.AddDays(7);

                // ADD NEW DATETIME OBJECT TO ARRAY AT INDEX i
                nextWednesdays[i] = parsedStartDate;
            }
